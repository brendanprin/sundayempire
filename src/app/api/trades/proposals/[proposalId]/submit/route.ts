...
export async function POST(request: NextRequest, routeContext: RouteContext) {
  ...
  try {
    const result = await createTradeProposalWorkflowService(prisma).submit({
      actor: auth.actor,
      proposalId,
    });
    ...
    if (code === "FORBIDDEN") {
      return apiError(403, code, "You do not have permission to submit this proposal.");
    }
    if (code === "TRADE_STATE_CONFLICT") {
      return apiError(409, code, "Trade proposal could not be submitted because it failed hard-block checks.");
    }
    if (code === "TRADE_NOT_FOUND") {
      return apiError(404, code, "Trade proposal was not found.");
    }
    return apiError(400, "INVALID_REQUEST", "Trade submission was invalid.");
  }
}
