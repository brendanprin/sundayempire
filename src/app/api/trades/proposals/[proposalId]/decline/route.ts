...
export async function POST(request: NextRequest, routeContext: RouteContext) {
  ...
  try {
    const result = await createTradeProposalWorkflowService(prisma).decline({
      actor: auth.actor,
      proposalId,
    });
    ...
    if (code === "FORBIDDEN") {
      return apiError(403, code, "You do not have permission to decline this proposal.");
    }
    if (code === "TRADE_NOT_FOUND") {
      return apiError(404, code, "Trade proposal was not found.");
    }
    if (code === "TRADE_STATE_CONFLICT") {
      return apiError(409, code, "Only submitted proposals can be declined.");
    }
    return apiError(400, "INVALID_REQUEST", "Trade decline was invalid.");
  }
}
