...
export async function POST(request: NextRequest, routeContext: RouteContext) {
  ...
  try {
    const result = await createTradeProposalSettlementService(prisma).settle({
      actor: auth.actor,
      proposalId,
    });
    ...
    await recordPilotEventSafe(prisma, {
      ...
      actor: auth.actor,
      eventType: PILOT_EVENT_TYPES.TRADE_PROCESSED,
      eventCategory: "trade",
      eventStep: "settlement",
      status: "success",
    ...
    if (code === "FORBIDDEN") {
      return apiError(403, code, "You do not have permission to settle this trade proposal.");
    }
    ...
    if (code === "TRADE_STATE_CONFLICT") {
      return apiError(
        409,
        code,
        "Only accepted or review-approved proposals can be settled, and hard-blocked proposals cannot be processed.",
      );
    }
    return apiError(400, "INVALID_REQUEST", "Trade settlement was invalid.");
  }
}
